...
Status (2026-09-18): Phase 1 is complete and on `main`: the store, the page editor, the canvas panel and split view, storage wiring with autosave and per-page canvas views, backup download and restore, notebook settings, the left rail and the bare notebook switcher. Phase 2 is under way: text formatting (the page editor on ProseMirror with bold, italic, colour and alignment, a floating format bar), the notebook cover (colour, emoji and subtitle in notebook settings, a swatch in the app bar and the switcher), zine pages (the second page kind, the media block with its grid presets, text below and beside, image import with downscaling into the shared files table) and the media pool (the panel's pool mode on zine pages, usage per image, placing by drag or click, drag onto the canvas, deletion refused while in use) are done; the backup format is at version 4. Next are themes, then Dark (specified in sections 2, 3 and 5 on 2026-09-18). Visual refinement is deliberately left for the end; the page's paper look should be settled before export work starts.
...
- Backup file = notebook, pages, canvas and files as one JSON document, images base64 inside. A zip backup with images as separate files is planned for Phase 3, for notebooks heavy with photos. Format version 4; version 1 files (plain-string columns) are converted on open, each line break becoming a paragraph boundary, files from before version 3 get the default cover, and pages from before version 4 are lined. The IndexedDB schema has the same upgrades (Dexie versions 2 to 4).
