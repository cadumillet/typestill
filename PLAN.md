...
- Under the page or spread, centred on it, sits the page bar: previous, the page counter, next; then new page (lined or zine), page settings and delete page. It belongs to the page the way a caption does, so it moves with the page when the desk is resized, and the page is fitted above it.
...
- Clippings: a drawing taken from the canvas, or an image from the clipboard, placed on a lined page as a free object that overlaps the text without touching it. The text does not flow around a clipping (the user leaves gaps or writes over it as they like); a clipping sits over the text or behind it, is moved by dragging and resized by its corners keeping its proportions, may run past the page edge, and is deleted from its tools or with Backspace while selected. Clippings are part of the page: they are in thumbnails, exports and the preview, and their images live in the notebook's files table with the media pool's images, counted as uses. A page keeps any number of them. Zine pages do not take clippings for now.
...
- Clip to page: while elements are selected on the canvas and a lined page is open, a control in the panel's corner renders the selection as an SVG with no background and places it on the page as a clipping (see Pages). The drawing stays on the canvas; the clipping is a copy. Pasting on a page also accepts what Excalidraw's own "Copy as SVG" and "Copy as PNG" put on the clipboard, and any other image.
...
- The rail holds nothing else: the page controls live in the page bar under the page.
...
Page {
  ...
  clippings: Clipping[]                // lined pages: free objects over or under the text; empty otherwise
...
Clipping {
  id: string
  fileId: string                       // an SVG or PNG in the notebook's files table, content-hashed like every image
  x: number                            // mm from the page's left edge
  y: number                            // mm from the page's top edge
  width: number                        // mm; the height follows the image's proportions
  layer: "over" | "under"              // over the text, or between the rules and the text
}
...
NotebookFile { notebookId, id, data: BinaryFileData }   // images and clippings, referenced by zine pages, clippings and the canvas
```
...
- Highlight: the `highlight` mark with four tints shared with the canvas, themed per paper, translucent like a marker; a highlighter button with tint swatches in the format bar and Cmd+Shift+H; backup version 7, Dexie version 8
- Clippings: free objects over or under a lined page's text, from the canvas ("Clip to page" on a selection, an SVG with no background) or the clipboard (SVG or PNG); moved by drag, resized from the corners keeping proportions, sent behind or brought in front, deleted; in thumbnails, exports and the media pool; backup version 8, Dexie version 9
