...
- The page bar and the aligned app bar: the page controls (previous, counter, next, new page, page settings, delete page) move from the rail and the app bar to a bar under the page or spread, centred on it; the rail keeps only its squares and the filter; the app bar keeps the wordmark at the left and a right-hand group (cover swatch and name, search, notebook menu, preview, panel toggle) whose right edge lines up with the page's right edge
