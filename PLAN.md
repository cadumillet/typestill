...
Status (2026-09-18): Phase 1 is complete and on `main`: the store, the page editor, the canvas panel and split view, storage wiring with autosave and per-page canvas views, backup download and restore, notebook settings, the left rail and the bare notebook switcher. Phase 2 is under way: text formatting (the page editor on ProseMirror with bold, italic, colour and alignment, a floating format bar), the notebook cover (colour, emoji and subtitle in notebook settings, a swatch in the app bar and the switcher) and zine pages (the second page kind, the media block with its grid presets, text below and beside, image import with downscaling into the shared files table) are done; the backup format is at version 4. Next are the media pool and themes (specified in sections 2, 3 and 5 on 2026-09-17 and 2026-09-18). Visual refinement is deliberately left for the end; the page's paper look should be settled before export work starts.
