...
Status (2026-09-18): Phase 1 is complete and on `main`: the store, the page editor, the canvas panel and split view, storage wiring with autosave and per-page canvas views, backup download and restore, notebook settings, the left rail and the bare notebook switcher. Phase 2 is under way: text formatting (the page editor on ProseMirror with bold, italic, colour and alignment, a floating format bar), the notebook cover, zine pages, the media pool and themes (the theme model with the built-in Ruled and Plain, a theme field on the notebook and a picker in settings, runtime baseline measurement, the zine text font as a theme slot) are done; the backup format is at version 5. Next is Dark with the app appearance setting, then the divider polish, tags, thumbnails, the two-page spread, the date stamp and page number, and the page settings panel. Visual refinement is deliberately left for the end; the page's paper look should be settled before export work starts.
