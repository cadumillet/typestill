...
- One notebook in the UI: the switcher and the shelf are no longer reachable; opening a backup replaces the current notebook after asking; the name moves into notebook settings; the code for multiple notebooks stays and is listed in section 8
...
- Multiple notebooks (descoped from the UI 2026-09-18, Phase 5): the shelf screen (`src/notebook/Shelf.tsx`, its CSS and test), the notebook switcher (`src/notebook/NotebookSwitcher.tsx`), the session's `Shelf` state with `closeNotebook`, `openNotebook`, `createNotebook` (outside the first run), `renameNotebook` (from the shelf; the settings dialog owns renaming now) and `deleteNotebook`, and the store's `listNotebooks`, `deleteNotebook` and the notebook-scoped records. The storage model stays multi-notebook by design (section 3), so sync and a future shelf need no format change. The first-run welcome dialog and the backup replace flow are the only ways a notebook is created.
