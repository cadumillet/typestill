...
Status: Phase 0 spike done. Direction revised on 2026-09-17 (see the decisions log). Phase 1 in progress.
...
- Text on the left, canvas on the right, with a draggable divider. Inspired by a split editor view.
- The text side shows one page, fit to the available height at the paper's aspect ratio.
- Either side can be collapsed. Canvas collapsed: two-page spread of consecutive pages. Text collapsed: full-width canvas.
...
- Optional divider: a light gray vertical line that splits the page into two columns of text. It defaults to the middle of the writable area (from the margin line to the right edge), snaps to 10mm steps when moved, and can be removed. A new page inherits the divider of the page it was created from.
...
- Clear view: the page without rules, margin and divider, read-only.
...
Page {
  ...
  columns: string[]                    // one or two plain-text columns
...
- Our own page editor, plain DOM, in Excalifont self-hosted from Excalidraw's font files
- Dexie for IndexedDB
...
- Store: notebooks, pages, canvas, files, backup
- Page editor: lines, margin, capacity, one column
- Canvas: infinite Excalidraw with the grid toggle
- Split view with draggable divider and collapse on either side
- Multiple pages: next / prev / new page; canvas view per page
- IndexedDB autosave for pages and canvas
