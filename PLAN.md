...
Status (2026-09-18): Phase 1 is complete and on `main`: the store, the page editor, the canvas panel and split view, storage wiring with autosave and per-page canvas views, backup download and restore, notebook settings, the left rail and the bare notebook switcher. Phase 2 is under way: text formatting, the notebook cover, zine pages, the media pool, themes (Ruled and Plain), Dark with the app appearance setting and the divider drag are done; the backup format is at version 5. Next are tags, thumbnails, the two-page spread, the date stamp and page number, and the page settings panel. Visual refinement is deliberately left for the end; the page's paper look should be settled before export work starts.
