...
Status (2026-09-18): Phases 1 and 2 are complete and on `main`, and Phase 3 is nearly so. Phase 1: the store, the page editor, the canvas panel and split view, storage wiring with autosave and per-page canvas views, backup download and restore, notebook settings, the left rail and the bare notebook switcher. Phase 2: text formatting, the notebook cover, zine pages, the media pool, themes (Ruled and Plain), Dark with the app appearance setting, the divider drag, tags, hover thumbnails, the two-page spread, the date stamp and page number, and the page settings popover with the margin line. Phase 3 so far: full-text search, PDF and PNG exports, the zip backup with storage figures and pruning, page deletion, keyboard shortcuts and the first-run welcome; the backup format is at version 5 in both its JSON and zip shapes. Left: the shelf screen. Visual refinement is deliberately left for the end.
